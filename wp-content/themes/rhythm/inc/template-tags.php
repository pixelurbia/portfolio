...
function rhythm_get_social_links( $pattern = '%s', $category = '', $style = 1 ) {
	...
	if ( $custom_query->have_posts() ):
		...
		while ( $custom_query -> have_posts() ) :
			...
			if ( $style == 3 ) {
				$out .= sprintf( $pattern, '<a href="' . esc_url( get_the_title() ) . '" target="_blank"><span class="mn-soc-link tooltip-bot" title="' . esc_attr( ucfirst( str_replace( 'fa-', '', ts_get_post_opt( 'icon' ) ) ) ) . '"><i class="fa ' . esc_attr( ts_get_post_opt( 'icon' ) ) . '"></i></span></a> ');
			} elseif ( $style == 2 ) {
				$out .= sprintf( $pattern, '<a href="' . esc_url( get_the_title() ) . '" target="_blank"><span class="mn-soc-link tooltip-top" title="' . esc_attr( ucfirst( str_replace( 'fa-', '', ts_get_post_opt( 'icon' ) ) ) ) . '"><i class="fa ' . esc_attr( ts_get_post_opt( 'icon' ) ) . '"></i></span></a> ' );
			} else {
				$out .= sprintf( $pattern, '<a href="' . esc_url( get_the_title() ) . '" target="_blank"><i class=" fa ' . esc_attr( ts_get_post_opt( 'icon' ) ) . '"></i></a> ');
			}
