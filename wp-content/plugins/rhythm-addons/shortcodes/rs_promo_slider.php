...
function rs_promo_slide( $atts, $content = '', $id = '' ) {
  ...
  if ( $alt == 'yes' ) {
	  ...
	  $output .= '</div>';
	  $output .= '</div>';
	  $output .= '<h1 class="hs-line-6 no-transp font-alt white">' . esc_html( $small_heading ) . ' <a href="' . esc_url( $href ) . '" class="btn btn-mod btn-w btn-round" style="margin-top:-3px;">' . esc_html( $btn_text ) . '</a></h1>';
	  $output .='</div>';
	  $output .= '</div><!--//.row-->';
